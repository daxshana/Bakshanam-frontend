...
import AddHomeMaker from '../component/HomeMakerManagement';
...
const HomeMakerManagement = () => {
    ...
    const handleDeleteHomeMaker = async (homeMakerId) => {
        console.log("Deleting homemaker with ID:", homeMakerId); // Debug log
        try {
            const response = await axios.delete(`http://localhost:5004/api/home-makers/deleteHomeMaker/${homeMakerId}`);
            console.log("Home maker deleted:", response.data);
            // Update state or perform any other actions needed after deletion
        } catch (error) {
            console.error('Error deleting homemaker:', error);
            alert('Failed to delete home maker. Please try again.');
        }
    };
    useEffect(() => {
    ...
    return (
        <div className="container mt-4">
            <h1>Homemakers Management</h1>
            {errorMessage && <Alert variant="danger">{errorMessage}</Alert>}
            <AddHomeMaker onAddHomeMaker={handleAddHomeMaker} initialData={currentHomeMaker} />
            <HomeMakerList 
                homeMakers={homeMakers} 
                onEditHomeMaker={handleEditHomeMaker} 
                onDeleteHomeMaker={handleDeleteHomeMaker} // Make sure this prop is passed down correctly
            />
        </div>
    );
};
